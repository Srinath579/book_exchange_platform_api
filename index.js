...
app.get('/userrequests', verifyToken, async (req, res) => {
  ...
  try {
    let connection = await pool.getConnection();
    let [userrequests] = await connection.query('SELECT BOOKS.AUTHOR, BOOKS.BOOK_IMAGE, BOOKS.BOOK_CONDITION, BOOKS.OWNER_NAME, EXCHANGE.* FROM EXCHANGE INNER JOIN BOOKS WHERE EXCHANGE.REQUESTER_NAME = ? AND EXCHANGE.REQUESTER_EMAIL_ID = ? AND BOOKS.OWNER_EMAIL_ID = EXCHANGE.OWNER_EMAIL_ID AND BOOKS.TITLE = EXCHANGE.BOOK_NAME', [username, emailid]);
    await connection.release();
...
app.get('/getexchangerequests', verifyToken, async (req, res) => {
  ...
  try {
    let connection = await pool.getConnection();
    let [exchangerequests] = await connection.query('SELECT BOOKS.AUTHOR, BOOKS.BOOK_IMAGE, BOOKS.BOOK_CONDITION, BOOKS.OWNER_NAME, EXCHANGE.* FROM EXCHANGE INNER JOIN BOOKS WHERE EXCHANGE.OWNER_EMAIL_ID = ? AND BOOKS.OWNER_EMAIL_ID = EXCHANGE.OWNER_EMAIL_ID AND BOOKS.TITLE = EXCHANGE.BOOK_NAME', [emailid]);
    await connection.release();
...
app.put('/submitexchangerequest', verifyToken, async (req, res) => {
  ...
  try {
    ...
    await connection.release();
    res.json({ message: 'Inserted a New Exchange Request Successfully' });
  } catch (error) {
...
app.put('/adduserbook', verifyToken, async (req, res) => {
  const { useremailid, username } = req.headers;
  const formData = req.body.formData;
  try {
    let connection = await pool.getConnection();
    let [users] = await connection.query('SELECT * FROM USERS WHERE EMAIL_ID=?', [useremailid])
    let [exchangerequests] = await connection.query('INSERT INTO BOOKS(TITLE, AUTHOR, GENRE, BOOK_CONDITION, AVAILABILITY_STATUS, BOOK_IMAGE, OWNER_ID, OWNER_EMAIL_ID, OWNER_NAME) VALUES(?,?,?,?,?,?,?,?,?)', [formData.title, formData.author, formData.genre, formData.bookcondition, formData.status, formData.bookimage, users[0].USER_ID, useremailid, username]);
    await connection.release();
    res.json({ message: 'Inserted a New Book Successfully' });
  } catch (error) {
    console.error('Error in Inserting a Book:', error);
    res.status(500).json({ error: 'Internal server error' });
  }
});
